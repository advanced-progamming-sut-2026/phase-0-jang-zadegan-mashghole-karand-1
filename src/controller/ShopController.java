...
public class ShopController {
    ...
    public CommandResult buy(String itemId, int quantity) {
        ...
        if (itemId.equals("daily")) {
            return buyDailyDeal(user, quantity);
        }
        ...
        return switch (itemId) {
            case "pot" -> buyPot(user, item, quantity);
            case "seed pack random" -> buySeedPackRandom(user, item, quantity);
            case "plant food" -> buyPlantFood(user, item, quantity);
            case "gem to coin" -> buyGemToCoin(user, item, quantity);
            default -> failure("Unknown shop item.");
        };
    }

    private CommandResult buyDailyDeal(User user, int quantity) {
        shop.ensureDailyFresh();
    ...
    private CommandResult buyPot(User user, ShopItem item, int quantity) {
    ...
    private CommandResult buySeedPackRandom(User user, ShopItem item, int quantity) {
        int totalCost = item.getPrice() * quantity;
        if (user.coins < totalCost) {
            return failure("Not enough coins.");
        }
    ...
    private CommandResult buyPlantFood(User user, ShopItem item, int quantity) {
    ...
    private CommandResult buyGemToCoin(User user, ShopItem item, int quantity) {
        int totalCost = item.getPrice() * quantity;
        if (user.gems < totalCost) {
            return failure("Not enough gems.");
        }
