...
public class AppEventHandler {
    ...
    private void onZombieSpawned(ZombieSpawnedEvent event) {
        ...
        controller.sendMessage("Zombie "+ event.zombie.type.name+" spawned in lane "+ event.zombie.row+" which costed "+ event.zombie.type.baseStats.wavePointCost+".");
    ...
    private void onZombieDied(ZombieDiedEvent  event) {
        if(event == null || event.zombie == null || event.zombie.type == null) return;
        Zombie zombie = event.zombie;
        controller.sendMessage("Zombie of type "+zombie.type.name+" is dead at ("+zombie.position.x+", "+zombie.position.y+").");
    }

    private void onGlowingZombieDied(GlowingZombieDiedEvent event){
        if(event == null) return;
        controller.sendMessage("The glowing zombie dropped a plant food; you have "+storage.getCurrentUser().plantFood+" plant foods now.");
    }

    private void onSunDropped(SunDroppedEvent event){
        if(event == null || event.sun == null) return;
        controller.sendMessage("New "+event.sun.type.name()+" sun is dropping at position ("+event.sun.position.x+", "+event.sun.targetY+")");
    }

    private void onSunLanded(SunLandedEvent event){
        if(event == null || event.sun == null) return;
        controller.sendMessage("Sun reached the ground at position ("+event.sun.position.x+", "+event.sun.targetY+")");
    }
}
